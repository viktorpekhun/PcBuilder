...
namespace Scraping.Infrastructure.Scrapers
{
    public class GpuScraper : IComponentScraper<Gpu>
    {
        ...
        public async Task<ScrapingResult<Gpu>> ScrapeAsync(string url, HttpClient client, ConcurrentBag<Gpu> componentsFromDb, ConcurrentBag<Store> storesFromDb, CancellationToken cancellationToken = default)
        {
            ...
            var gpu = new Gpu { HotlineUrl = url };
            ...
            if (edgesOffers != null)
            {
                ...
                var avgPrice = offers.Any() ? Math.Round(offers.Average(p => p.Price)) : 0; gpu.AveragePrice = (decimal)avgPrice;
