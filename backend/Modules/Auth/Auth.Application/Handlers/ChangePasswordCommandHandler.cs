    ...
    public class ChangePasswordCommandHandler : IRequestHandler<ChangePasswordCommand, Result<string>>
    {
        ...
        public async Task<Result<string>> Handle(ChangePasswordCommand request, CancellationToken cancellationToken)
        {
            ...
            if (string.IsNullOrEmpty(user.PasswordHash))
                return Result<string>.Failure(new Error("NoPassword", "This account uses Google sign-in and has no password.", 400));

            if (!BCrypt.Net.BCrypt.Verify(request.CurrentPassword, user.PasswordHash))
                return Result<string>.Failure(new Error("WrongPassword", "Current password is incorrect.", 400));
