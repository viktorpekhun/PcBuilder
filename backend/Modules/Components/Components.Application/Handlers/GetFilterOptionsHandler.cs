...
{
    public class GetFilterOptionsHandler : IRequestHandler<GetFilterOptionsQuery, Dictionary<string, List<string>>>
    {
        private readonly IApplicationDbContext _context;

        public GetFilterOptionsHandler(IApplicationDbContext context)
        {
            _context = context;
        }

        public async Task<Dictionary<string, List<string>>> Handle(GetFilterOptionsQuery request, CancellationToken cancellationToken)
        {
            ...
            switch (request.ComponentType)
            {
                case ComponentType.Cpu:
                    {
                        var components = await _context.Set<Cpu>().ToListAsync(cancellationToken);
                ...
                case ComponentType.Gpu:
                    {
                        var components = await _context.Set<Gpu>().ToListAsync(cancellationToken);
                ...
                case ComponentType.Motherboard:
                    {
                        var components = await _context.Set<Motherboard>().ToListAsync(cancellationToken);
                        var brands = components.Select(c => c.Brand).Distinct().OrderBy(b => b).ToList();
                        result["brand"] = brands;
                        var sockets = components.Select(c => c.Socket).Distinct().OrderBy(b => b).ToList();
                        result["socket"] = sockets;
                ...
                case ComponentType.CpuCooler:
                    {
                        var components = await _context.Set<CpuCooler>().ToListAsync(cancellationToken);
                        var brands = components.Select(c => c.Brand).Distinct().OrderBy(b => b).ToList();
                        result["brand"] = brands;
                ...
                case ComponentType.PcCase:
                    {
                        var components = await _context.Set<PcCase>().ToListAsync(cancellationToken);
                ...
                case ComponentType.Ram:
                    {
                        var components = await _context.Set<Ram>().ToListAsync(cancellationToken);
                        var brands = components.Select(c => c.Brand).Distinct().OrderBy(b => b).ToList();
                        result["brand"] = brands;
                ...
                case ComponentType.Ssd:
                    {
                        var components = await _context.Set<Ssd>().ToListAsync(cancellationToken);
                        var brands = components.Select(c => c.Brand).Distinct().OrderBy(b => b).ToList();
                        result["brand"] = brands;
                ...
                case ComponentType.Hdd:
                    {
                        var components = await _context.Set<Hdd>().ToListAsync(cancellationToken);
                        var brands = components.Select(c => c.Brand).Distinct().OrderBy(b => b).ToList();
                        result["brand"] = brands;
                        var formFactors = components.Select(c => c.FormFactor).Distinct().OrderBy(b => b).ToList();
                        result["formFactor"] = formFactors.Where(ff => ff != null).Cast<string>().ToList();
                ...
                case ComponentType.Fan:
                    {
                        var components = await _context.Set<Fan>().ToListAsync(cancellationToken);
                        var brands = components.Select(c => c.Brand).Distinct().OrderBy(b => b).ToList();
                        result["brand"] = brands;
