...
export const componentSpecFullConfigs: Record<string, FullSpecConfig[]> = {
    cpu: [
        ...
        { key: 'tpd', label: 'Максимальне тепловиділення, Вт', unit: '' },
        {
            ...
            format: (values) => {
                if (values.integratedGraphics === true) return 'є';
                if (values.integratedGraphics === false) return 'немає';
                return '';
            },
            unit: ''
        },
    ...
    motherboard: [
        ...
        { key: 'pcleX1Quantity', label: 'Слоти PCI-Express x1', unit: '' },
        {
            ...
            format: (values) => {
                if (values.powerMotherboard) return `1x${values.powerMotherboard} pin`;
                return '';
            },
            unit: ''
        },
        ...
        { key: 'formFactor', label: 'Форм фактор', unit: '' },
        {
            ...
            format: (values) => {
                if (values.sizeDimentions) {
                    return String(values.sizeDimentions)
                        .replace(' мм', '').replace('мм', '').replaceAll(' ', '')
    ...
    pcCase: [
        ...
        { key: 'usb', label: 'USB на передній панелі', unit: '' },
        {
            ...
            format: (values) => {
                if (values.sizeDimentions) {
                    return String(values.sizeDimentions).replace(' мм', '').replace('мм', '');
                }
                return '';
            },
            unit: 'мм'
    ...
    cpuCooler: [
        { key: 'brand', label: 'Бренд', unit: '' },
        {
            keys: ['type'],
            label: 'Тип',
            format: (values) => values.type === 'Water' ? 'Водяне' : 'Повітряне',
            unit: ''
        },
        ...
        { key: 'maxPowerDissipation', label: 'Розсіювана потужність', unit: 'Вт' },
        {
            keys: ['noiseLevelDb'],
            label: 'Рівень шуму',
            format: (values) => values.noiseLevelDb ? `${values.noiseLevelDb}` : '',
            unit: 'Дб'
        },
        {
    ...
    powerSupply: [
        ...
        { key: 'efficiencyStandart', label: 'Стандарт', unit: '' },
        {
            ...
            label: 'ККД',
            format: (values) => values.efficiencyPercent ? `${values.efficiencyPercent}%` : '',
            unit: ''
        },
